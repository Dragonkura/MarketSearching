...
namespace GFramework.GUI
{
    ...
    public class GUIHandlerBase : MonoBehaviour
    {
        ...
        public bool processBackKey = true;
        ...
        public virtual GameObject InitExc()
        {
            try
            {
                ...
                {
                    obj = GameObject.Instantiate(guiPrefabObj) as GameObject;
                }
                else
                {
                    if (guiPrefabObj != null)
                    {
                        obj = PrefabUtility.InstantiatePrefab(guiPrefabObj) as GameObject;
                    }
                    else
                    {
                        var targetName = guiName + ".prefab";
                        var destinationPath = Path.Combine("Assets/Prefabs/GUI/", targetName);
                        var asset = AssetDatabase.LoadAssetAtPath(destinationPath, typeof(GameObject)) as GameObject;
                        obj = PrefabUtility.InstantiatePrefab(asset) as GameObject;
                    }
                }
#else
                ...
                if (guiCanvas != null)
                {
                    guiCanvas.renderMode = RenderMode.ScreenSpaceCamera;
        ...
        public bool Init()
        {
            if (status >= GUIStatus.Ready && gui != null)
                return true;
        ...
        {
            if (status == GUIStatus.Invalid || gui == null)
            {
                Init();
            }

            //if (guiCanvas != null) guiCanvas.enabled = true;
        ...
        public virtual void HideWithoutAnimation(params object[] @parameter)
        {
            if (guiObject == null)
                return;
        ...
        private void ForceHide(params object[] @parameter)
        {
            if (guiObject == null) return;

            if (onBeginHidding != null) onBeginHidding();

            var subAnims = guiObject.GetComponentsInChildren<GUIAnimation>();
        ...
        public void ForceDestroy(params object[] @parameter)
        {
            if (guiObject == null) return;
            if (!gui.selfHide)
            {
        ...
        public void ForceDestroyWithoutEvent(params object[] @parameter)
        {
            if (guiObject == null) return;
            Destroy(guiObject);
            status = GUIStatus.Invalid;
        ...
        {
            return status == GUIStatus.Showing;
        }

        public bool IsHiding()
        {
            return status == GUIStatus.Hiding;
        }
